...
public class BinaryHttpResponseHandler extends AsyncHttpResponseHandler {
    ...
    protected void handleMessage(Message msg) {
        ...
        switch(msg.what) {
            ...
            case FAILURE_MESSAGE:
                response = (Object[])msg.obj;
                handleFailureMessage((Throwable)response[0], response[1] instanceof byte[] ? (byte[])response[1] : null);
                break;
            default:
    ...
    void sendResponseMessage(HttpResponse response) {
        ...
        /*for(String anAllowedContentType : mAllowedContentTypes) {
            if(anAllowedContentType.equalsIgnoreCase(contentTypeHeader.getValue()) || contentTypeHeader.getValue().toLowerCase().indexOf(anAllowedContentType) != -1) {
                foundAllowedContentType = true;
            }
        }*/
        if (contentTypeHeader != null && contentTypeHeader.getValue() != null) {
			if (-1 != contentTypeHeader.getValue().toLowerCase().indexOf("image/")) {
				foundAllowedContentType = true;
			}
		}
