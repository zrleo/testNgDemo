...
public class HttpConnection {
	...
	public static String getResultContentWithMultiFormData(String url, Map<String, String> params, String resourceName,
	  String resource) {
		...
		try {
			...
			output.writeBytes(PREFIX + BOUNDARY + LINEND);
			...
			// 
			output.writeBytes(LINEND + PREFIX + BOUNDARY + PREFIX + LINEND);
			output.flush();
			// 
	...
	private static void addFormField(Map<String, String> params, DataOutputStream output) throws IOException {
		...
		for (Map.Entry<String, String> entry : params.entrySet()) {
			...
			sb.append("Content-Disposition: form-data; name=\"").append(entry.getKey()).append("\"").append(LINEND).append(LINEND);
			sb.append(entry.getValue());
			sb.append(LINEND).append(PREFIX);
			sb.append(BOUNDARY);
			sb.append("LINEND");
		}
	...
	private static void addImageContent(String paramsName, String fileName, DataOutputStream output) throws IOException {
		...
		if (file.length() > 0) {
			...
			sb.append("Content-Transfer-Encoding: binary").append(LINEND).append(LINEND);
			output.writeBytes(sb.toString());
