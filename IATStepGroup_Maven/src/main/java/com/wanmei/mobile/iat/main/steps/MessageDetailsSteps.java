	 ...
	 public void assertMessageDetail(JSONArray jsonArray,List<List<String>> resultSet)throws Exception
	 {
		 for(int i=0;i<jsonArray.size();i++)
		 {
			 ...
			 if(!md.id.equals(0))
			 {
			     ...
			     Assert.assertEquals(md.subject, NgaHelper.ReEncodeDataFromDB(resultSet, index, 1),"subjects are different!!");
			     Assert.assertEquals(md.content,NgaHelper.ReEncodeDataFromDB(resultSet, index, 2),"contents are different!!");
