...
public class PostQuoteSteps {
	@StepMethodDesc(description="postQuote,nga interface 17",owner="Ruiyun.Ren")
	public void postQuote(String parameterID)throws Exception{	
		InterfaceStepParameter parameter=(InterfaceStepParameter)ParameterHelper.getInterfaceStepParameter(parameterID);
		JSONObject actual=RequestHelper.requestSender(Constant.Url.PostQuote,setParam(parameter));
	...
	private LinkedHashMap<String, String>setParam(InterfaceStepParameter parameter) throws Exception
	{
		...
		String sqlCommand="select fid,tid from pw_threads order by rand() limit 1";
		List<List<String>>resultSet=DataBaseHelper.getResult(Constant.DB_URL_KA, sqlCommand);
		String tid1=resultSet.get(1).get(1);
